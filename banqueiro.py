...
def imprimir_os_dados(recursos_totais, matriz_alocados, recursos_alocados, recursos_disponiveis,
                      matriz_recursos_necessarios):
...
def algoritmo_banqueiro(qtd_processos, qtd_tipos_recursos, recursos_disponiveis, matriz_recursos_necessarios,
                        matriz_alocados):
    ...
    while np.count_nonzero(rodando) > 0:
        ...
        for num_processo in range(qtd_processos):
            ...
            if rodando[num_processo] != 0:
                if all(i >= 0 for i in recursos_disponiveis - (
                        matriz_recursos_necessarios[num_processo] - matriz_alocados[num_processo])):
